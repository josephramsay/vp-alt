...
echo kubectl config set-context --current --namespace=$TARGET_NAMESPACE
...
echo kubectl delete secret sh.helm.release.v1.${RELEASE_NAME}.v1 --ignore-not-found
# test with:
#   helm template --debug $RELEASE_NAME $SCRIPT_DIR
echo helm upgrade --install --debug $RELEASE_NAME $SCRIPT_DIR
